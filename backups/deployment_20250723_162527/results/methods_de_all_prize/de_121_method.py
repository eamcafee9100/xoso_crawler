...
class De121TwoDigitMethod(BasePredictionMethod):
    """
    Phương pháp 121 Đề 2 số
    Dự đoán từ giải 4.4 (số thứ 2 và bóng) và giải 7.2 (số thứ 2 và bóng)
    """
    ...
    def get_description(self) -> str:
        return "Dự đoán 2 số từ giải 4.4 và giải 7.2"
    
    def calculate(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        giai_4_4 = data.get('giai_4_4', '')
        giai_7_2 = data.get('giai_7_2', '')
        
        if not giai_4_4 or not giai_7_2:
            logger.warning(f"[{self.get_code()}] Thiếu dữ liệu đầu vào: giai_4_4={giai_4_4}, giai_7_2={giai_7_2}")
            return {'two_digits_special': []}
        
        try:
            # Số thứ nhất từ giải 4.4: số thứ 2 và bóng
            # Ví dụ: 2496 -> số thứ 2 = 4, bóng là 9
            if len(giai_4_4) >= 2:
                digit_second = giai_4_4[1]
            ...
            logger.error(f"[{self.get_code()}] Lỗi tính 121 đề 2 số: {str(e)}")
            return {'two_digits_special': []}

class De121ThreeDigitMethod(BasePredictionMethod):
    """
    Phương pháp 121 Đề 3 số
    Dự đoán từ giải 4.4 (số thứ 2 và bóng), giải 7.2 (số thứ 2 và bóng), và giải 3.1 (càng đề)
    """
    def get_code(self) -> str:
        return "de121_3d"
    ...
    def get_description(self) -> str:
        return "Dự đoán 3 số từ giải 4.4, 7.2 và giải 3.1 (càng đề)"
    ...
    def calculate(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if not giai_4_4 or not giai_7_2 or not giai_3_1:
            logger.warning(f"[{self.get_code()}] Thiếu dữ liệu đầu vào: giai_4_4={giai_4_4}, giai_7_2={giai_7_2}, giai_3_1={giai_3_1}")
        ...
        try:
            ...
            logger.error(f"[{self.get_code()}] Lỗi tính 121 đề 3 số: {str(e)}")
            return {'three_digits_special': []}