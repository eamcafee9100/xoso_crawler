...
class De58TwoDigitMethod(BasePredictionMethod):
    """
    Phương pháp 58 Đề 2 số
    Dự đoán từ giải 2.2 (tổng số thứ 1 và 2 và bóng) và giải 6.1 (tổng số thứ 1 và 2 và bóng)
    """
    def get_code(self) -> str:
        return "de58_2d"
    
    def get_name(self) -> str:
        return "58 Đề 2 số (giống 52)"
    ...
    def calculate(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            # Số thứ 2 từ giải 6.1: tổng số thứ 1 và số thứ 2 và bóng
            ...
            logger.error(f"[{self.get_code()}] Lỗi tính 58 đề 2 số: {str(e)}")
            return {'two_digits_special': []}


class De58ThreeDigitMethod(BasePredictionMethod):
    """
    Phương pháp 58 Đề 3 số
    Dự đoán từ giải 2.2, 6.1 và giải 2.1 (chạm đề)
    """
    def get_code(self) -> str:
        return "de58_3d"
    
    def get_name(self) -> str:
        return "58 Đề 3 số (giống 52 khác càng)"
    
    def get_description(self) -> str:
        return "Dự đoán 3 số từ giải 2.2, 6.1 và giải 2.1 (chạm đề)"
    ...
    def calculate(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if not giai_2_2 or not giai_6_1 or not giai_2_1:
            logger.warning(f"[{self.get_code()}] Thiếu dữ liệu đầu vào: giai_2_2={giai_2_2}, giai_6_1={giai_6_1}, giai_2_1={giai_2_1}")
        ...
        try:
            ...
            if len(giai_2_1) >= 4:
                cham_de = giai_2_1[3]  # Lấy số thứ 4, ví dụ: 18519 -> 1
            ...
            logger.error(f"[{self.get_code()}] Lỗi tính 58 đề 3 số: {str(e)}")
            return {'three_digits_special': []}