...
from results.models import DanDeDacBiet, KetQuaXoSo,PredictionResult
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['clean_all']:
            from results.models import DanDeDacBiet, PredictionResult
            
            self.stdout.write(self.style.WARNING("Xóa tất cả dữ liệu dự đoán cũ..."))
            
            # Lấy ID của tất cả DanDeDacBiet trước khi xóa
            dan_de_ids = list(DanDeDacBiet.objects.values_list('id', flat=True))
            
            # Xóa tất cả PredictionResult
            prediction_count = PredictionResult.objects.filter(dan_de_id__in=dan_de_ids).count()
            PredictionResult.objects.filter(dan_de_id__in=dan_de_ids).delete()
            
            # Xóa tất cả DanDeDacBiet
            dande_count = DanDeDacBiet.objects.count()
            DanDeDacBiet.objects.all().delete()
            
            self.stdout.write(self.style.SUCCESS(f"Đã xóa {dande_count} bản ghi DanDeDacBiet và {prediction_count} bản ghi PredictionResult"))
        ...
        dande_days = DanDeDacBiet.objects.filter(analysis_date__gte=start_date, analysis_date__lte=end_date).count()
            ...
            for current_date in batch:
                try:
                    ...
                    if KetQuaXoSo.objects.filter(ngay=current_date).exists():
                        ...
                        dande_exists = DanDeDacBiet.objects.filter(analysis_date=current_date).exists()
                        
                        if not dande_exists or force:
                            self.stdout.write(f"Đang xử lý ngày {current_date}...")
                            # Sử dụng atomic cho mỗi ngày riêng biệt
                            with transaction.atomic():
                                result = DanDeDacBiet.create_for_date(current_date)
