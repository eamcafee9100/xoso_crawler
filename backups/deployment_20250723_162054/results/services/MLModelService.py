    ...
    def train_models(self, force_retrain=False):
        ...
        try:
            ...
            if success:
                ...
                self._models_loaded = False  # Reset flag to force reload
                self._ensure_models_loaded()
...
# 🚀 LAZY SINGLETON: Không tạo instance ngay khi import
# Instance sẽ được tạo khi được gọi lần đầu tiên
def get_ml_model_service():
    """Get ML Model Service instance (lazy loading)"""
    if not hasattr(get_ml_model_service, '_instance'):
        get_ml_model_service._instance = MLModelService()
    return get_ml_model_service._instance