        ...
        context.update(
            {
                ...
                "method_analysis": self._get_method_analysis_for_js(
                    start_date, end_date
                ),
                ...
                for session in sessions_by_date[current_date]:
                    ...
                    for method_result in method_results:
                        ...
                        if not isinstance(method_id, int) or method_id <= 0:
                            logger.error(f"❌ Invalid method_id in database: {method_id} (type: {type(method_id)})")
                            continue
                        ...
                        try:
                            session_method_data = {
                                "method_id": int(method_result.method.id),  # ✅ Ensure int
                                "method_name": str(method_result.method.name),
                                "category": str(method_result.method.get_category_display()),
                                "confidence": float(method_result.overall_confidence),
                                "predicted_numbers": list(method_result.base_prediction_numbers) if method_result.base_prediction_numbers else [],
                                "tracking_results": tracking_results,
                            ...
                            # ✅ FINAL VALIDATION
                            if isinstance(session_method_data["method_id"], int) and session_method_data["method_id"] > 0:
                                session_methods_data.append(session_method_data)
                            else:
                                logger.error(f"❌ Failed final validation for method_id: {session_method_data['method_id']}")
                                
                        except (ValueError, TypeError) as e:
                            logger.error(f"❌ Error creating session method data for method {method_result.method.id}: {e}")
                            continue
...
def api_pattern_analysis(request):
    ...
    try:
        ...
        active_methods = PredictionMethod.objects.filter(is_active=True).values_list('id', flat=True)
            ...
            for method_id in list(pattern_predictions.keys()):
                try:
                    ...
                    if method_id_str in pattern_predictions:
                        ...
                        pattern_predictions[method_id_str]["day_predicted_numbers"] = day_predicted_numbers
...
def _get_method_predicted_numbers_for_days(method_id, analysis_date, target_date):
    ...
    try:
        ...
        if isinstance(method_id, str):
            ...
            if not method_id.isdigit():
                logger.error(f"❌ Invalid method_id format: '{method_id}' (expected integer or numeric string)")
                return day_predicted_numbers
            method_id_int = int(method_id)
        elif isinstance(method_id, int):
            if method_id <= 0:
                logger.error(f"❌ Invalid method_id value: {method_id} (must be positive integer)")
                return day_predicted_numbers
            method_id_int = method_id
        elif isinstance(method_id, float):
            if not method_id.is_integer() or method_id <= 0:
                logger.error(f"❌ Invalid method_id float: {method_id} (must be positive integer)")
                return day_predicted_numbers
            method_id_int = int(method_id)
        else:
            logger.error(f"❌ Invalid method_id type: {type(method_id)} (expected int or numeric string)")
            return day_predicted_numbers

        # ✅ VALIDATE dates
        if not isinstance(analysis_date, date) or not isinstance(target_date, date):
            logger.error(f"❌ Invalid date types: analysis_date={type(analysis_date)}, target_date={type(target_date)}")
            return day_predicted_numbers
        ...
        logger.info(f"🔍 Getting predicted numbers for method {method_id_int} ({method.name})")
                ...
                if method_result:
                    ...
                    if predicted_nums and isinstance(predicted_nums, list):
                        ...
                        if clean_numbers:
                            ...
                            for day in range(1, 4):
                                prediction_target_date = target_date + timedelta(days=day-1)
                                
                                try:
                                    # Giới hạn 10 số tốt nhất
                                    day_predicted_numbers[f"day_{day}"] = clean_numbers[:10]
                                    logger.debug(f"   Day {day} ({prediction_target_date}): {len(clean_numbers[:10])} numbers")
                                        
                                except Exception as day_error:
                                    logger.error(f"   Error processing day {day} numbers: {day_error}")
                                    day_predicted_numbers[f"day_{day}"] = []
                        else:
                            logger.warning(f"   ❌ No valid numbers after cleaning for method {method_id_int}")
                    else:
                        logger.warning(f"   ❌ No valid predicted numbers for method {method_id_int}")
                        
                else:
                    logger.warning(f"   ❌ No method result found for method {method_id_int} in session {session.session_id}")
        ...
        logger.error(f"❌ Unexpected error getting predicted numbers for method {method_id}: {e}")
    ...
    logger.info(f"📊 Result for method {method_id}: day_1={total_day1}, day_2={total_day2}, day_3={total_day3}")
