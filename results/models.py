...
class NumberAnalysisCache(models.Model):
    """Cache kết quả phân tích tần suất cho hiệu suất"""
    
    number = models.CharField(max_length=2, db_index=True, help_text="Số 2 chữ số (00-99)")
    ...
    max_gan_start_date = models.DateField(null=True, blank=True, help_text="Ngày bắt đầu gan tối đa")
    max_gan_end_date = models.DateField(null=True, blank=True, help_text="Ngày kết thúc gan tối đa")
    ...
    # Số đính kèm (xuất hiện cùng ngày)
    ...
    # Phân tích chuỗi tiền nhiệm (số xuất hiện trước)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["number", "analysis_date"]),
            models.Index(fields=["updated_at"]),
        ]
    
    def __str__(self):
        return f"Analysis for {self.number} on {self.analysis_date}"


class NumberAnalysisDetail(models.Model):
    """Chi tiết phân tích heart tại thời điểm cụ thể"""
    
    cache = models.ForeignKey(NumberAnalysisCache, on_delete=models.CASCADE, related_name='details')
    detail_type = models.CharField(max_length=20, choices=[
        ('gan_history', 'Lịch sử gan'),
        ('cycle_pattern', 'Mẫu chu kỳ'),
        ('companion_detail', 'Chi tiết số đính kèm'),
        ('consecutive_events', 'Sự kiện liên tiếp'),
    ])
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["cache", "detail_type"]),
        ]
