...
class De26TwoDigitMethod(BasePredictionMethod):
    """
    Phương pháp 26 Đề 2 số
    ...
    def calculate(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.error(f"[{self.get_code()}] Lỗi tính 26 đề 2 số: {str(e)}")
            return {'two_digits_special': []}

class De26ThreeDigitMethod(BasePredictionMethod):
    """
    Phương pháp 26 Đề 3 số
    Dự đoán từ giải 2.1 (tổng số thứ 1 và 2, lấy bóng), giải 5.5 (tổng số thứ 3 và 4, lấy bóng), và giải 3.4 (càng đề từ số thứ 3)
    """
    def get_code(self) -> str:
        return "de26_3d"
    ...
    def get_description(self) -> str:
        return "Dự đoán 3 số từ giải 2.1 (tổng số thứ 1 và 2), giải 5.5 (tổng số thứ 3 và 4), và giải 3.4 (càng đề từ số thứ 3)"
    ...
    def calculate(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if not giai_2_1 or not giai_5_5 or not giai_3_4:
            logger.warning(f"[{self.get_code()}] Thiếu dữ liệu đầu vào: giai_2_1={giai_2_1}, giai_5_5={giai_5_5}, giai_3_4={giai_3_4}")
        ...
        try:
            ...
            # Càng đề từ giải 3.4: số thứ 3 và bóng
            # Ví dụ: 14674 -> số thứ 3 = 6, bóng là 1
            if len(giai_3_4) >= 3:
                digit_third_cang = giai_3_4[2]
                cang_nums = [digit_third_cang, str(get_ball_number(int(digit_third_cang)))]
            else:
            ...
            logger.error(f"[{self.get_code()}] Lỗi tính 26 đề 3 số: {str(e)}")
            return {'three_digits_special': []}