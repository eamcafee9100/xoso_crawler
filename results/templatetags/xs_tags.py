...
@register.filter
def format_number_2digit(row, col):
    """Format two digits to create 2-digit number string"""
    try:
        return f"{row}{col}"
    except:
        return "00"
...
def lookup(dictionary, key):
    """Template filter để truy cập dynamic key trong dictionary"""
...
def get_item(dictionary, key):
    """Template filter để truy cập dictionary item bằng dynamic key"""
    if isinstance(dictionary, dict):
        return dictionary.get(str(key), {})
    return {}

@register.filter
def add_day_prefix(day):
    """Thêm prefix 'day_' vào số ngày"""
    return f"day_{day}"
