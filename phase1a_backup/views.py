...
# ============================================================================
# 🎯 KÉP LỆCH 3-DAY PREDICTION VIEWS
# ============================================================================


@login_required
def kep_lech_3day_dashboard(request):
    """
    🎯 Dashboard chính cho Kép Lệch 3-Day Prediction
    """
    try:
        # Import template predictor
        import os
        import sys
        ...
        method_info = predictor.get_method_info()

        # Lấy dữ liệu gần đây để hiển thị
        from_date = timezone.now().date() - timedelta(days=7)
        to_date = timezone.now().date()
        recent_data = predictor.get_historical_data(from_date, to_date)

        context = {
            "method_info": method_info,
            "recent_data": recent_data,
            "total_records": len(recent_data.get("raw_records", [])),
            "current_date": timezone.now().date(),
            "page_title": "Kép Lệch 3-Day Prediction Dashboard",
        }

        return render(
            request, "predictions_tracker/kep_lech_3day_dashboard.html", context
        )

    except Exception as e:
        logger.error(f"❌ Kép Lệch dashboard error: {e}")
        messages.error(request, f"Lỗi tải dashboard: {str(e)}")
        return render(
            request,
            "predictions_tracker/kep_lech_3day_dashboard.html",
            {"error": str(e), "page_title": "Kép Lệch 3-Day Prediction Dashboard"},
        )


@login_required
@csrf_exempt
def run_kep_lech_3day_analysis(request):
    """
    🚀 API chạy phân tích Kép Lệch 3-Day
    """
    if request.method != "POST":
    ...
    try:
        ...
        if analysis_date_str:
            ...
            analysis_date = timezone.now().date() - timedelta(days=4)  # Default

        # Chạy phân tích
        result = predictor.run_analysis_demo(analysis_date)
        ...
        if result["success"]:
            ...
            response_data = {
                ...
                "strategy_analysis": session.strategy_analysis,
                "evaluation_results": None,
            }

            # Nếu có kết quả đánh giá
            if (
                hasattr(session, "session_success")
                and session.session_success is not None
            ):
                response_data["evaluation_results"] = {
                    "session_success": session.session_success,
                    "winning_day": session.winning_day,
                    "total_hit_count": session.total_hit_count,
                    "win_rate": round(session.win_rate, 4),
                    "roi_percentage": round(session.roi_percentage, 1),
                        ...
                        day_results.append(
                            {
                                ...
                                "actual_giai_db": day_result.actual_giai_db,
                                "actual_giai_7": day_result.actual_giai_7,
                                "winning_numbers": day_result.winning_numbers,
                                "hit_count": day_result.hit_count,
                                "hit_rate": round(day_result.hit_rate, 4),
                                "is_winning_day": day_result.is_winning_day,
                            }
                        )

                response_data["evaluation_results"]["day_results"] = day_results
        ...
        logger.error(f"❌ Kép Lệch analysis API error: {e}")
        return JsonResponse({"success": False, "error": str(e)})


@login_required
@csrf_exempt
def run_kep_lech_multi_session(request):
    """
    🚀 API chạy phân tích đa phiên Kép Lệch
    """
    if request.method != "POST":
        return JsonResponse({"success": False, "error": "Method not allowed"})
    ...
    try:
        ...
        # Lấy tham số từ request
        data = json.loads(request.body) if request.body else {}
        session_count = data.get("session_count", 5)
        ...
        # Tạo danh sách ngày phân tích
        demo_dates = []
        for i in range(session_count):
            demo_date = start_date + timedelta(days=i * 3)  # Cách nhau 3 ngày
            demo_dates.append(demo_date)

        # Chạy phân tích cho từng phiên
        all_sessions = []
        session_summaries = []
            ...
            try:
                ...
                if result["success"] and "session" in result:
                    ...
                    session_summary = {
                        ...
                        "confidence_score": round(session.confidence_score, 3),
                        "success": getattr(session, "session_success", None),
                        "roi_percentage": getattr(session, "roi_percentage", 0),
                        "winning_day": getattr(session, "winning_day", None),
                    }
                    ...
                    session_summaries.append(
                        {
                            ...
                            "analysis_date": demo_date.strftime("%Y-%m-%d"),
                        }
                    )

            except Exception as e:
                logger.error(f"❌ Session {i} error: {e}")
                session_summaries.append(
                    {
                        "session_number": i,
                        "error": str(e),
        ...
        # Tạo báo cáo tổng hợp nếu có sessions thành công
        ...
        logger.error(f"❌ Multi-session analysis API error: {e}")
        return JsonResponse({"success": False, "error": str(e)})


@login_required
def kep_lech_3day_reports(request):
    """
    📊 Hiển thị báo cáo Kép Lệch 3-Day
    """
    try:
        ...
        single_reports = glob.glob(
            os.path.join(base_dir, "kep_lech_3day_report_*.json")
        )
        comprehensive_reports = glob.glob(
            os.path.join(base_dir, "kep_lech_comprehensive_report_*.json")
        )

        for file_path in single_reports:
        ...
        for file_path in comprehensive_reports:
            filename = os.path.basename(file_path)
            file_stats = os.stat(file_path)
            report_files.append(
                {
        ...
        context = {
            ...
            "page_title": "Kép Lệch 3-Day Reports",
        }

        return render(
            request, "predictions_tracker/kep_lech_3day_reports.html", context
        )

    except Exception as e:
        logger.error(f"❌ Reports view error: {e}")
        messages.error(request, f"Lỗi tải reports: {str(e)}")
        return render(
            request,
            "predictions_tracker/kep_lech_3day_reports.html",
            {"error": str(e), "page_title": "Kép Lệch 3-Day Reports"},
        )


@login_required
def view_kep_lech_report(request, filename):
    """
    ...
    try:
        ...
        if not os.path.exists(file_path):
            messages.error(request, f"File {filename} không tồn tại")
            return redirect("predictions_tracker:kep_lech_3day_reports")
        ...
        context = {
            ...
            "page_title": f"Báo cáo: {filename}",
        }

        return render(
            request, "predictions_tracker/kep_lech_report_detail.html", context
        )

    except Exception as e:
        logger.error(f"❌ View report error: {e}")
        messages.error(request, f"Lỗi đọc báo cáo: {str(e)}")
        return redirect("predictions_tracker:kep_lech_3day_reports")
